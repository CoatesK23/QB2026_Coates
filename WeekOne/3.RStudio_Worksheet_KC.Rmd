...
author: "Kelsey Coates; Z620: Quantitative Biodiversity, Indiana University"
...
rm(list=ls())
getwd()
#setwd("C:\\Users\\coatesk23\\Documents\\QuantBio\\QB2026_Coates\\WeekOne")
...
col1 <- rnorm(5, mean = 8, sd = 2)     # first column
col2 <- rnorm(5, mean = 25, sd = 10)   # second column
...
> Answer 1:
> The rnorm function generates random numbers that are normally distributed and the arguments allow the mean and standard deviation to be specified.
...
> Answer 2:
> 10 x 5 meaning 10 columns and five rows
...
```{r}
zoop <- read.table("zoops.txt", header = TRUE)
zoop_nuts <- read.table("zoop_nuts.txt", header = TRUE)
...
> Answer 3: 
> Zooplankton communities were positively correlated with TN, SRP, TIN, and ZIP. Zooplankton communities were negatively correlated with chlorophyll A. 
...
> Answer 4: 
> This correlation test also shows that zooplankton communities were positively correlated with TN, SRP, TIN, and ZIP and negatively correlated with chlorophyll A with about the same values as the first test considering rounding to the three digits. These results were not sensitive to parametric vs non-parametric methods. Parametric methods should be used over non-parametric methods when the data is normally distributed and measured on an interval/ratio scale. Non-parametric tests should be used when the data is not normally distributed and/or ordinal/rank data. Yes, Pearson's method is likely to have false positives if a correction for multiple comparisons is not completed.  FDR is important because large correlation matrices that are common in ecological data are likely to produce false positives due to the t-test's way of testing the null hypothesis of each variable independently.    
...
```{r}
fitreg <- lm(ZP ~ TN, data = zoop_nuts)
summary(fitreg)

newTN <- seq(min(zoop_nuts$TN), max(zoop_nuts$TN), 10)
...
p <- plot(zoop_nuts$TN, zoop_nuts$ZP, ylim = c(0, 10), xlim = c(500, 5000), 
     xlab = expression(paste("Total Nitrogen (", mu,"g/L)")),
     ylab = "Zooplankton Biomass (mg/L)", las = 1) + text(zoop_nuts$TN, zoop_nuts$ZP, zoop_nuts$NUTS, pos = 3, cex = 0.8) + lines(newTN, regline) + matlines(newTN, conf95[,c("lwr", "upr")], type= "l", lty = 2, lwd = 1, col = "black")
...
> Answer 5:
> The regression model shows that total nitrogen is a significant predictor of zooplankton communities with a pvalue of 1.91e-05. 
...
```{r}
NUTS <- factor(zoop_nuts$NUTS, levels = c('L', 'M', 'H'))

zp.means <- tapply(zoop_nuts$ZP, NUTS, mean)
...
zp.sem <- tapply(zoop_nuts$ZP, NUTS, sem)

bp <- barplot(zp.means, ylim =c(0, round(max(zoop_nuts$ZP), digits = 0)), 
...
fitanova <- aov(ZP ~ NUTS, data = zoop_nuts)
...
> Answer 6:
> Zooplankton taxa varied in abundance by site. SIMO shown in pink was present in most sites with a relatively high abundance. BOSM shown in coral was a rare species not present in many sites and had low abundance when present. 
...
zoops_trim_df <- zoops_trim_df %>% mutate(Site = c(1:24))
df_long <- as.data.frame(zoops_trim_df) %>% mutate(Site = rownames(.)) %>%
...
```{r}
system('git config --global user.email "coatesk23@students.ecu.edu" ')
system('git config --global user.name "CoatesK23" ')

system(' git add 3.RStudio_Worksheet_KC.Rmd" ')
system('git commit -m "Add 3.RStudio_Worksheet_KC.Rmd" ')
